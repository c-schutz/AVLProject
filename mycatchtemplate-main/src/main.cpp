...
int main(){
    ...
    for(int i = 0; i < numOfCommands; i++){
        ...
        for(int j = 0; j < (int) command.size(); j++){
            if (char(command[j]) == ' ' && (char(command[j + 1]) == ' ')){ //make sure there aren't multiple spaces in a row
                std::cout << "unsuccessful" << std::endl;
                break;
            }
            if(char(command[j]) == '"'){ //if the command is a name then don't separate at the space
                while(j < (int) command.size()){
                    j++;
                    if(!isalpha(char(command[j])) && char(command[j]) != ' ' && char(command[j]) != '"'){//if any of the characters between the parenthesis aren't alphabetical or a space
                        commandList[0] = " ";
                        break;
                    }
                    if(char(command[j]) == '"'){
                        ...
                        break;
                    }else if(j == (int) command.size()){//if the parenthesis are never closed break and make sure it returns unsuccessful
                        commandList[0] = " ";
                        break;
                    }
                }
                ...
                index = j + 1;
                continue;
            }else if(j == ((int) command.size()) - 1){//this is for the last command because there is no space to remove
        ...
        if(commandList[0] == "insert"){
            ...
            if(commandList.size() > 3 || commandList[2].size() != 8){ //make sure there are the right amount of command and the ID is the right length
                std::cout << "unsuccessful" << std::endl;
                continue;
            }
